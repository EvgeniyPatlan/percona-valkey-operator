...
func TestClusterRoundTrip(t *testing.T) {
	...
	orig := &v1.PerconaValkeyCluster{
		...
		Spec: v1.PerconaValkeyClusterSpec{
			...
			Auth: &v1.AuthSpec{Enabled: ptr(true), PasswordSecret: v1.UserPasswordSecret{Name: "c-users", Keys: []string{"password"}}},
			TLS: &v1.TLSConfig{
				SecretName:     "tls",
...
func TestFieldPresence(t *testing.T) {
	...
	checks := map[reflect.Type][]string{
		reflect.TypeOf(v1.PerconaValkeyClusterSpec{}): {
			...
			"auth", "disableCommands", "expose", "networkPolicy", "env", "extraEnvVars",
			"serviceAccountName", "automountServiceAccountToken", "podSecurityContext",
		...
		reflect.TypeOf(v1.ValkeyNodeSpec{}): {
			...
			"env", "extraEnvVars", "serviceAccountName", "automountServiceAccountToken",
