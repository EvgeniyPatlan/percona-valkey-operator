...
- **Auto-stamping.** On first reconcile, if `spec.crVersion` is empty, `CheckNSetDefaults` stamps it from `version.txt`'s `major.minor` (mirroring Percona's `pkg/k8s/utils.go` / `*_defaults.go` auto-stamp). As-built this is `stampCrVersion()` in `perconavalkeycluster_defaults.go`, which writes `version.MajorMinor()` when `spec.crVersion` is empty. A freshly applied CR therefore always converges to the running operator's contract.
- **Version gating.** Throughout the controllers, behaviour that depends on the contract is guarded by the `pkg/version` helpers — never by hardcoded checks. As-built these are package functions: `version.CompareVersion(target)` compares the **operator's own** `major.minor` against a target (returning -1/0/+1, patch ignored), and `version.CompareMajorMinor(a, b)` compares two version strings for the crVersion acceptance / monotonicity logic. New fields and new reconcile behaviour added in `1.1` are only activated when the contract permits, so an unchanged `1.0` CR keeps `1.0` behaviour even under a `1.1` operator until the user opts in.
- **Accepted-set computation.** `version.AcceptedCrVersions("x.y")` computes the crVersion values the operator will reconcile (its own minor plus the immediately-preceding released minor), implementing the §8 compatibility matrix in code rather than a hardcoded table. The last accepted value is mirrored on `status.lastObservedCrVersion`, which the runtime monotonicity check (§7) compares the incoming `spec.crVersion` against to reject downgrades.
- **Patch immateriality.** `crVersion` is `major.minor`; operator patch releases (`1.1.0 → 1.1.1`) never change it, so bug-fix operator upgrades never trigger a CR API migration (`version.CompareVersion` ignores patch by design).
...
> **As-built interface (`pkg/version/service/types.go`).** The controller depends on a `RecommendedImageResolver` interface (repository pattern) — `Resolve(ctx, VSRequest) (*VSResponse, error)` — so it can unit-test against a fake; the concrete HTTP client supplies the implementation. The request (`VSRequest`) carries primitives only (no `pkg/apis` import, to keep `pkg/version` a near-leaf): `Product`, `OperatorVersion`, `CrVersion`, `CurrentEngine`, `KubernetesVersion`, `Platform`, and the resolved `Apply` policy. The response (`VSResponse`) returns `Recommended` and `Latest` **`VersionSet`** triples — `{Engine, Exporter, Backup}` image tags validated **together** — which is how the "multi-image compatibility" guarantee is enforced in code: all three move as a unit so a recommended engine never pairs with an incompatible exporter or backup tool. `Resolve` returns a **typed** error distinguishing a reachable-but-erroring service from an unreachable endpoint, so the once-per-window poll skips cleanly on a transient outage (below) without rolling or degrading. The default endpoint is `service.DefaultEndpoint = "https://check.percona.com"`, overridden per cluster by `spec.upgradeOptions.versionServiceEndpoint`.
...
> **As-built status (v1alpha1): the `v1` API and conversion webhook are DEFERRED to GA graduation — NOT implemented in this release.** The operator currently serves **only** `v1alpha1`. No `api/v1` hub package, no `ConvertTo`/`ConvertFrom` spoke logic, and no `Hub()` markers ship today (grounding: `cmd/manager/manager.go` lines 53–62 document the deferral; there is no `conversion.Convertible` implementation anywhere in the tree). The **only** piece wired now is the **webhook serving-cert startup gate** — `waitForWebhookCertGate` in `cmd/manager/main.go` (the reusable scaffold in `pkg/tls/webhookcert.go`), which is a **guarded no-op unless `WEBHOOK_CERT_SECRET` is set**. When the API is field-stable, the conversion lands as: (1) an `api/v1` hub package with `Hub()` markers on all four kinds; (2) `v1alpha1` spoke `ConvertTo`/`ConvertFrom` with lossless round-trip via the `valkey.percona.com/conversion-data` annotation (crVersion carried verbatim); (3) controllers re-pointed to the `v1` hub; (4) the conversion webhook handler registered behind the cert gate that already passes; (5) the CRD `conversion.strategy: Webhook` + `caBundle` wiring in `deploy/`. **The remainder of this section is the forward design for that GA work, not a description of current behaviour.**
...
3. **Conversion webhook (GA work — not yet shipped).** Register a conversion webhook (at GA, `deploy/` will add the `CustomResourceDefinition.spec.conversion.strategy: Webhook` wiring and the webhook Service/Certificate). The API server invokes it to convert between served versions on read/write. The webhook's TLS cert is provisioned by cert-manager (preferred) or a secret ref, consistent with [Security Architecture](07-security.md). Today, only the startup **cert-readiness gate** for this webhook is wired (see the as-built status banner above and [Security Architecture](07-security.md) §3.5); the conversion handler itself is deferred.
...
7. **Dual-serving during graduation (DEFERRED to GA — forward design).** As-built the operator serves **only** `v1alpha1`; the planned `v1alpha1` (storage) + `v1` (served) coexistence — `v1alpha1` writes store as-is, `v1` reads return a non-persisted converted copy, the operator works on the hub in memory and tolerates both stored versions, round-trip envtest-validated — ships with the `v1` GA promotion (§6).
8. **`v1alpha1 → v1` via hub-and-spoke conversion webhook (DEFERRED to GA).** Lossless round-trip, storage flip + one-time migration before dropping `v1alpha1` — none of this conversion logic is implemented today; only the webhook serving-cert **readiness gate** is wired (`cmd/manager/main.go`, guarded by `WEBHOOK_CERT_SECRET`). See the §6 as-built status banner.
