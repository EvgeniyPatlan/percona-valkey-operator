...
| Auth | `_exporter` ACL user, password from `internal-<cluster>-system-passwords` Secret | least privilege — strictly read-only introspection (`+info +cluster\|info +latency +ping` only; see the ACL block below) |
...
> **API note.** The v1alpha1 `ExporterSpec` defined in [API & CRD Design](03-api-design.md) §2.9 carries `enabled`, `image`, `resources`, **`port`** (`*int32`, default `9121`), **`scrapeInterval`** (`string`, default `20s`), and **`tls`** (`*ExporterTLSSpec` with `tls.enabled bool`, default `false`). All are fully implemented as part of the v1alpha1 schema. `port` backs the named `metrics` container port and the static `PodMonitor`/`ServiceMonitor` target; `scrapeInterval` is templated into the `PodMonitor`/`ServiceMonitor`; `tls.enabled` switches the exporter to serve `/metrics` over HTTPS using the cluster's cert family and flips the generated monitor to `scheme: https`. Keep [03-api-design.md](03-api-design.md) §2.9 as the field-level source of truth; the `spec.exporter.<field>` paths referenced below are the real schema.

The `_exporter` ACL is the operator's canonical least-privilege policy (denies `@all`, then re-adds only the introspection commands the exporter issues), generated only when `spec.exporter.enabled` is true. It is reproduced **verbatim** from the canonical copy in `pkg/valkey/acl.go` (mirrored in [Security Architecture](07-security.md) §4.3) so all copies stay in lockstep:

```text
resetchannels resetkeys -@all +info +cluster|info +latency +ping
```

This is the canonical least-privilege `_exporter` ACL — **read-only introspection only**. It re-adds exactly four commands on top of `-@all`: `+info` (engine `INFO`), `+cluster|info` (the `CLUSTER INFO` summary), `+latency` (latency history), and `+ping` (health). It deliberately does **not** grant `+get`/`+eval`/`+scan`, so the credential **cannot read keyspace data, run Lua, or scan keys** — it is strictly read-only and cannot mutate topology or reset its own auth. The earlier broad redis_exporter "recommended" policy (which included `+get`/`+eval`/`+scan` for optional per-key metrics) is **not** what this operator ships; key-level exporter metrics are out of scope for the `_exporter` user. See [Security Architecture](07-security.md) §4.3 for the full ACL and system-user model.
...
- **`INFO`** is the dominant cost. Most sections are O(1), but `INFO keyspace`/full `INFO` work is **O(keyspace)** in the number of databases/keys it must summarise, so on a very large keyspace the periodic `INFO` is the single most expensive thing the exporter does. (Per-key/`+get`/`+scan` checks are **not** possible with this operator's `_exporter` ACL — see §2.4 — so the exporter never adds per-key cost.)
...
**Recommended scrape interval: ~20s.** This is the `spec.exporter.scrapeInterval` default (§2.4) and the value baked into the static `PodMonitor` (§3.2); it matches the operator's own 30s steady-state requeue cadence without over-scraping. **Do not run aggressive sub-10s intervals (e.g. 5s) on large keyspaces** — an O(keyspace) `INFO` every 5s competes with client traffic on the single-threaded engine and can measurably raise p99 command latency for no monitoring benefit (Prometheus alerting windows here are minutes, not seconds). Tune via `spec.exporter.scrapeInterval`; lower it only on small instances where `INFO` is demonstrably cheap.

**ACL is read-only introspection.** The `_exporter` user is a **strictly read-only introspection credential** — its canonical ACL (`resetchannels resetkeys -@all +info +cluster|info +latency +ping`, §2.4) re-adds only `INFO`/`CLUSTER INFO`/`LATENCY`/`PING` and never mutates state, reads keyspace data, or runs Lua (this is the least-privilege design in [Security Architecture](07-security.md) §4.3). There is no `+get`/`+eval`/`+scan` to drop — the operator never grants them.

**Typical memory footprint: ~10–50 Mi.** A core-mode `redis_exporter`-compatible sidecar steady-states at roughly **10–50 Mi** RSS (toward the low end for a single instance, higher with many tracked command stats). The §2.4 default `64Mi` request comfortably covers this.
...
Ship a **`PodMonitor`** as the default discovery resource because metrics are exposed *per pod* and the operator already stamps per-pod shard/node labels — a `PodMonitor` scrapes each pod directly and preserves those labels natively. A **`ServiceMonitor`** variant is the opt-in alternative for environments that route all scraping through Services (or that lack pod-level RBAC for the Prometheus instance), pointed at a dedicated headless metrics Service.

> **As-built: static manifests, not Go-typed CRs.** The operator deliberately does **not** depend on the prometheus-operator Go types — the `PodMonitor`/`PrometheusRule` are shipped as **static YAML** under `config/prometheus/` (`podmonitor.yaml`, `podmonitor-tls.yaml`, `prometheusrule.yaml`) rather than being reconciled by a controller. The `<cluster>`/`<namespace>` placeholders are filled by the `valkey-db` Helm chart or by hand; the in-tree base targets a cluster named `valkey-sample`. Apply them only when the Prometheus Operator (`monitoring.coreos.com` CRDs) is installed — they are not created automatically, so clusters without the Prometheus Operator never get orphaned CRs. The exporter sidecar itself is still gated on `spec.exporter.enabled`.
...
A 20s interval with a 10s timeout matches the operator's own 30s steady-state requeue cadence (see [Control Plane & Reconciliation](04-control-plane.md)) without over-scraping; it is the `spec.exporter.scrapeInterval` default (§2.4) baked into the static `config/prometheus/podmonitor.yaml`.

### 3.3 Metrics TLS

When `spec.exporter.tls.enabled` is `true` (the single boolean of `ExporterTLSSpec`; see the API note in §2.4 and [03-api-design.md](03-api-design.md) §2.9), the exporter serves its `/metrics` endpoint over TLS **reusing the cluster's TLS cert family** (the same material provisioned for the cluster bus/client when `spec.tls` is set) — there is no separate per-exporter certificate-Secret field. With metrics TLS on, switch to the static `config/prometheus/podmonitor-tls.yaml` variant, which sets `scheme: https` and the matching `tlsConfig`. Cert provenance follows the cluster's `spec.tls` choice:

- **cert-manager mode (recommended):** the metrics serving cert is minted from the cluster's cert-manager `Issuer`/`ClusterIssuer` and auto-rotates; the `PodMonitor` references the CA via `tlsConfig.ca` sourced from the same Secret family. Consistent with [Security Architecture](07-security.md).
- **secret-ref mode:** the cluster's bring-your-own TLS Secret supplies `ca.crt`/`tls.crt`/`tls.key`. No auto-rotation; a cert change requires an exporter restart (the exporter is a separate container, so this does **not** roll the Valkey server).

The `podmonitor-tls.yaml` `tlsConfig` sources the CA from the `valkey-<cluster>-metrics-tls` Secret (the in-tree base uses `valkey-sample-metrics-tls`):
...
Expose `/metrics` only to the monitoring namespace. This is now an **operator-managed** `NetworkPolicy` driven by `spec.networkPolicy` (see [03-api-design.md](03-api-design.md) §2.10d and [Security Architecture](07-security.md) §7), gated on `spec.networkPolicy.enabled`, with `spec.networkPolicy.monitoringNamespace` selecting the namespace whose Prometheus pods may scrape the exporter (empty ⇒ the cluster's own namespace). In addition to the data-plane rules, it permits ingress to the exporter port **only** from pods carrying the Prometheus identity label in the configured monitoring namespace:
...
This confines the exporter port's blast radius to the monitoring stack — defence in depth on top of the already strictly read-only `_exporter` ACL (§2.4).
...
flowchart LR
    ...
    pm["PodMonitor (static YAML)<br/>config/prometheus/podmonitor.yaml<br/>(selector: valkey.percona.com/cluster)"]
    prom["Prometheus<br/>(scrape :9121, relabel<br/>cluster/shard_index/node_index)"]
    rules["PrometheusRule (static YAML)<br/>config/prometheus/prometheusrule.yaml<br/>(alert catalogue)"]
...
Shipped as the static `config/prometheus/prometheusrule.yaml` manifest (the `valkey-db` chart re-packages it; apply only when the Prometheus Operator is installed). Rules combine **engine metrics** (data plane, from the redis_exporter-compatible sidecar) with **operator state mirrored via kube-state-metrics custom-resource-state** (control plane: the `pvk_*` gauges and mirrored `Warning` events). The as-built catalogue is below — note the static file also ships two contract alerts not in the original sketch (`ValkeyObservedGenerationLag` on `pvk_metadata_generation - pvk_status_observed_generation`, and `ValkeyLeaderThrash` on repeated `redis_master_link_up` flips per shard):
...
| `ValkeyReconcileStuck` | `pvk_condition{type=Progressing,status=true}==1 and on(cluster) pvk_condition{type=Ready,status=true}!=1 for 30m` | warning | reconcile not converging (e.g. `LiveConfigApplied=ApplyFailed`) |
| `ValkeyObservedGenerationLag` | `(pvk_metadata_generation - pvk_status_observed_generation) > 0 for 15m` | warning | `status.observedGeneration` trails `metadata.generation`: the operator has not finished reconciling the latest spec (as-built contract alert; §4.1) |
| `ValkeyLeaderThrash` | `sum by(cluster,shard_index)(changes(redis_master_link_up[15m])) > 6 for 5m` | high | the replica→primary link flips repeatedly within 15m → failover thrash on a shard; role read from the engine, never `node_index` (§2.3). Correlate with `FailoverFailed` events (as-built) |
| `ValkeyFailoverFailing` | `rate(pvk_event_failover_failed_total[15m]) > 0 for 5m` | high | proactive failover not completing during a roll |
...
> Control-plane alerts (`ValkeyClusterNotReady`, `ValkeyClusterDegraded`, `ValkeyReconcileStuck`, `ValkeyObservedGenerationLag`, `ValkeyFailoverFailing`, `ValkeyBackup*`) require exposing `PerconaValkeyCluster`/`PerconaValkeyBackup` status and selected `Warning` events to Prometheus. The as-built `prometheusrule.yaml` expects a kube-state-metrics **custom-resource-state** config that emits `pvk_status_state{state}`, `pvk_condition{type,status}`, `pvk_metadata_generation`, `pvk_status_observed_generation`, `pvk_backup_last_success_timestamp_seconds`, plus event-derived counters `pvk_event_failover_failed_total` and `pvk_event_backup_failed_total` (via an event-exporter). The sketch rows above additionally reference `pvk_ready_shards`, `pvk_shards`, and `pvk_desired_replicas{shard_index}` for spec-derived targets like desired replica count. This is the bridge that makes the operator's first-class conditions/events alertable alongside engine metrics.
...
- [API & CRD Design](03-api-design.md) — `spec.exporter` (`enabled`/`image`/`resources`/`port`/`scrapeInterval`/`tls`), `spec.tls`, `spec.networkPolicy`, `status.conditions[]`, `status.state`, and the `app.kubernetes.io/*` / `valkey.percona.com/*` label contract (§6.1) used by `PodMonitor`/`ServiceMonitor` and dashboards.
