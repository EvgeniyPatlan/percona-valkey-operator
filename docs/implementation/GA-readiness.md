...
| **`expose.perPod` cluster-announce-ip — code-complete, NOT live-verified** | `reconcileExposeAnnounce` (expose_announce.go) sets per-node `AnnounceHost/Port` from the external Service addr; `valkeynode_resources.go` renders `--cluster-announce-ip/-port` from it (commit 256bbf8). Envtest-verified | Needs a real cloud **LoadBalancer** to confirm external redirect-following (kind has none). Validate on EKS/GKE before claiming GA support. |
| **Re-MEET gap on a simultaneous IP-changing restart (BUG #1) — FIXED** | live-found + fixed: when ALL pods restarted and changed IPs at once, nodes kept stale peer IPs in `nodes.conf` (`KnownNodes>1`, so not `IsIsolated()`) → `cluster_state:fail` and the operator never re-MEETed them. Fix: a gossip-repair reconcile step (`bootstrap.go repairStaleGossip` + a shared `meetAllReachable`; `NodeState.ClusterStateOK` now captured) re-MEETs all reachable nodes at their CURRENT pod IPs when not converged AND all pods Ready, gated on `cluster_state:ok` so a healthy cluster never re-MEETs | **DONE** — live-validated: autonomous re-convergence ~12–20s across 4 mass-restart rounds (incl. a partial 2-fail/4-ok partition), no steady-state churn, data preserved. |
