...
| **Re-MEET gap on a simultaneous IP-changing restart (BUG #1, UNFIXED)** | live-found: when ALL pods restart and change IPs at once, each node's persisted `nodes.conf` holds stale peer IPs; the operator only re-MEETs `IsIsolated()` (known_nodes≤1) nodes (`bootstrap.go` `meetIsolatedNodes`/`isolatedPendingNodes`), so populated-but-stale nodes are skipped → gossip can deadlock `cluster_state:fail` **non-deterministically** | **Fix before GA**: re-MEET nodes whose gossip view is stale/disconnected, not just isolated ones. Operational workaround: roll one pod at a time (never all at once). Both test clusters ultimately recovered (one self-healed, one needed a manual MEET). |
