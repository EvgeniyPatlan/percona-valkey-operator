...
| **OPS-x.5** | PR template + `CONTRIBUTING.md` assembly | Ship `.github/pull_request_template.md` carrying the §"Per-PR checklist", and assemble `CONTRIBUTING.md` from the GO/OPS section fragments into one canonical page linked from `README`. | `.github/pull_request_template.md`, `CONTRIBUTING.md`, `README.md` (link) | (markdown) checklist; contributing TOC | GO-x.1..x.7, OPS-x.1..x.4 | Template renders on a test PR; `CONTRIBUTING.md` cross-links resolve | link-check in `make docs`/CI (markdown link lint) | S (1) | Checklist going stale vs gates; OPS-x.6 audit keeps them in sync |
